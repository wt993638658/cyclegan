...
for epoch in range(n_epoch):
    ...
    for i ,batch in enumerate(dataloader):
        ...
        revocered_A = netG_B2A(fake_B)
        loss_cycle_ABA = loss_cycle(revocered_A,real_A) * 10.0
        revocered_B = netG_A2B(fake_A)
        loss_cycle_BAB = loss_cycle(revocered_B,real_B) * 10.0
    ...
    if (epoch+1)%20==0:
        if not os.path.exists("models"):
            os.makedirs("models")
        torch.save(netG_A2B.state_dict(),"models/netG_A2B"+str(epoch)+".pth")
        torch.save(netG_B2A.state_dict(),"models/netG_B2A"+str(epoch)+".pth")
        torch.save(netD_A.state_dict(),"models/netD_A"+str(epoch)+".pth")
        torch.save(netD_B.state_dict(),"models/netD_B"+str(epoch)+".pth")
